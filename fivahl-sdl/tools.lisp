...
(defparameter *rot13-base-down* (char-code #\a))
(defparameter *rot13-base-down-end* (char-code #\z))
(defparameter *rot13-base-up* (char-code #\A))
(defparameter *rot13-base-up-end* (char-code #\Z))
(defparameter *rot13-table-down* "nopqrstuvwxyzabcdefghijklm")
(defparameter *rot13-table-up* "NOPQRSTUVWXYZABCDEFGHIJKLM")

(defun in-a-z (c)
  (<= *rot13-base-down* (char-code c) *rot13-base-down-end*))

(defun in-a-z-up (c)
  (<= *rot13-base-up* (char-code c) *rot13-base-up-end*))

(defun rot13 (str)
  (with-output-to-string (result)
    (loop for c across str do
	 (princ (cond ((in-a-z c) (aref *rot13-table-down* (- (char-code c) *rot13-base-down*)))
		      ((in-a-z-up c) (aref *rot13-table-up* (- (char-code c) *rot13-base-up*)))
		      ((eql c #\Space) #\&)
		      ((eql c #\&) #\Space)
		      (t c))
		result))))
...
(defparameter *encode-base* (random 26))

(defun encode-generic (str fun)
  (with-output-to-string (result)
    (loop for c across str do
	 (princ (if (alpha-char-p c)
		    (if (lower-case-p c)
			(code-char (+ (mod (funcall fun (- (char-code c) *rot13-base-down*) *encode-base*) 26) *rot13-base-down*))
			(code-char (+ (mod (funcall fun (- (char-code c) *rot13-base-up*) *encode-base*) 26) *rot13-base-up*)))
		    c)
		result))))
...
(defun decode-string (str)
  (encode-generic str #'-))
;;; Encode decode string functions (end)



(defun forget-list (list)
  (if (> (length list) *forget-level*)
      (subseq list 1)
      list))
