...
(defun fn-end-write-content (tr)
  (format t "~&Write Content: ~A - Socket closed~&"
	  (transfer-name tr))
  (ignore-errors
...
(defun extract-content-from-tempfile (tempfile savedir fn-end-extract-content)
  (let* ((istream (open tempfile :direction :input
	 ...
	 (savename ""))
    (labels ((fn-end (tr)
		     (format t "~&Extract content from tempfile: ~A - Socket closed~&"
			     (transfer-name tr))
...
(defun parse-a-post-url-multipart (sock length tempfile savedir
  ...
  (let ((tempfile (format nil"~A~A~A" savedir tempfile (gensym))))
    (labels ((fn-end (tr)
		     (format t "~&Parse A Post Url Multipart: ~A - Socket closed~&"
			     (transfer-name tr))
...
(defun find-http-headers (headers str)
  ...
  (second (assoc str (fourth headers) :test #'string-equal)))


(defun only-head-type (type-request)
  (when (eql type-request :head)
      :head))
...
(defun fn-end-send-file (tr)
  (format t "~&Send file: ~A - Socket closed~&" (transfer-name tr))
  (ignore-errors
    (close (transfer-in tr)))
  (ignore-errors
    (close (transfer-out tr))))
    ...
    (loop for c across content
	 ...
	 with ignore = 0
       do (if (plusp ignore)
	      ...
	      (case c
		(#\+ (princ #\Space str))
		(#\% (let ((c (ignore-errors (code-char
					      (parse-integer
					       (format nil "~A~A" (aref content (1+ i))
						       (aref content (+ i 2)))
					       :radix 16)))))
		       (unless (or (equal c #\Return) (equal c #\Newline))
			 (princ c str)))
		     (setf ignore 2))
		(t (princ c str)))))
    str))
...
(defun write-http-content (headers filename)
  (unless (eql (second headers) :CONTENT)
    (format t "Sorry, headers content no data~%")
